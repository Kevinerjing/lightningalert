...
const profileDir = path.join(projectRoot, ".playwright", "google-drive-profile");
...
async function main() {
  ...
  try {
    await page.goto(driveUrl, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(2000);
    ...
    console.log("This script will automatically save the session after Google Drive opens.");
    ...
    while (Date.now() < deadline) {
      ...
      if (currentUrl.includes("drive.google.com") && !isAuthPage(currentUrl)) {
    ...
    if (!saved) {
      throw new Error("Google Drive did not finish opening before the session wait timed out.");
    }

    console.log(`Saved Google Drive session profile at: ${profileDir}`);
  } finally {
    await context.close();
  }
}
